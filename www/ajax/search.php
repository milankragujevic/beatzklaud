...
$cache = __DIR__ . '/cache/' . md5($query) . '.json';
if(!file_exists($cache) || time() - filemtime($cache) > 60 * 60) {
	$tracks = json_decode(file_get_contents('https://ws.audioscrobbler.com/2.0/?method=track.search&format=json&limit=30&page=1&api_key=' . $lastfm_key . '&track=' . rawurlencode($query)), true);
